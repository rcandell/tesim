...
C
      SETPT(3) = SETPT(3) + DXMV * 1.017 / 1000.
C
      ERROLD13 = ERR13
C
      RETURN
...
C
      SETPT(1) = SETPT(1) + DXMV * 5811. / 1000.
C
      ERROLD14 = ERR14
C
      RETURN
...
C
      SETPT(2) = SETPT(2) + DXMV * 8354. / 1000.
C
      ERROLD15 = ERR15
C
      RETURN
...
C
      SETPT(4) = SETPT(4) + DXMV * 15.25 / 1000.
C
      ERROLD17 = ERR17
C
      RETURN
...
C
      SETPT(10) = SETPT(10) + DXMV * 150. / 1000.
C
      ERROLD18 = ERR18
C
      RETURN
...
C
      SETPT(16) = SETPT(16) + DXMV  * 130. / 1000.
C
      ERROLD20 = ERR20
C
      RETURN
...
C============================================================================
      ...
      SUBROUTINE WRITEDAT(X,N)
        OPEN(UNIT=33,FILE='data/xmeas.dat',ACTION='write',STATUS='replace')     
        ! do the write
        CLOSE(33)          
        RETURN
        END
      
      SUBROUTINE WRITESTATES(XMEAS,XMV,YY,YP)

        CALL WRITEDAT(XMEAS,41)
        CALL WRITEDAT(XMV,12)
        CALL WRITEDAT(YY,50)
        CALL WRITEDAT(YP,50)
